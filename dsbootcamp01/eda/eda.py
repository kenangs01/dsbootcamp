...
#kategorik değişken olarak belirlediğim değişkenlerin özet bilgisi ve grafikleri çıkaracak fonksiyon tanımlıyorum

def cats_summary(data):
    import pandas as pd
    import seaborn as sns
    from matplotlib import pyplot as plt
    real_cat_cols = [col for col in data.columns if len(data[col].unique()) < 10] #değer sayısı 10 dan küçük değişkenleri ve nesne olan değişkenleri aldım
    for var in real_cat_cols:
        print(pd.DataFrame({var: data[var].value_counts(),
                       "Ratio": data[var].value_counts() * 100 / len(data[var])}), end = "\n\n\n") #seçtiğim değişkenlerle df oluşturuyorum ve değerlerinin oranlarını buluyorum
